...
describe("S3Utils", () => {
    ...
    after(async () => {
        if (hasAwsCredentials) {
            ...
            for (const fileName of fileNames) {
                fileDeletePromises.push(s3Client.deleteObject({ Bucket: s3Bucket, Key: fileName }).promise());
            }
    ...
    describe("getS3TarballUrlsAsync", () => {
        ...
        it("Should list package tarballs in the bucket with prefix 'prefix/'", if_awscreds(async () => {
        ...
        it("Should list package tarballs in the bucket without prefix", if_awscreds(async () => {
