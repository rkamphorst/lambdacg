...
class S3HandlerRepository implements HandlerRepositoryInterface {
    ...
    #s3Folder:S3Folder;
    #s3ObjectsUpToDate:{object:S3Object,tags:{key:string,value:string}[],isUpToDate:boolean}[]|undefined = undefined;
    ...
    async initializeWithLatestVersionsAsync(): Promise<void> {
        this.#s3ObjectsUpToDate =
            await Promise.all(
                (await this.#s3Folder.listLatestObjectVersionsAsync(/(\.tar.gz|\.tgz)$/))
                    .map(async s3Object => {
                        const tags = await s3Object.getTagsAsync();
                        return {
                            object: s3Object,
                            tags:tags,
                            isUpToDate: tags.findIndex(t => t.key === this.#updatedTagKey) >= 0
                        };
                    })
            );
    }
    ...
    getHandlerTarballStreamsAsync(): Promise<{ tarballName: string; stream: Readable; }[]> {
        ...
        return Promise.resolve(this.#s3ObjectsUpToDate.map(o => ({
    ...
    async markUpdatesAsync(updateMarker:string): Promise<void> {
        ...
        await Promise.all(
            this.#s3ObjectsUpToDate
                ...
                .map(x => x.object.setTagsAsync(x.tags.concat([{key:this.#updatedTagKey,value:updateMarker}])))
        );
    }
