...
describe("Executor", () => {
    it("Should throw if execution not supported", async () => {
        const handlerStub = sinon
            .stub()
            .returns({ responseProperty: "responsePropertyValue" });
        ...
        await expectToThrowAsync(
            () =>
                executeAsync("notSupported", [factory], "name", {
                    param1: "value1",
                }),
...
function executionShouldThrowIfHandlerThrows(execution: string): void {
    it(`Should throw if a handler throws`, async () => {
        ...
        await expectToThrowAsync(
            () =>
                executeAsync(execution, factories, "name", {
                    param1: "value1",
                }),
            (e) => e === thrownObject,
            "e is the thing thrown from the handler"
        );
...
function executionShouldThrowIfHandlerReturnsRejectedPromise(
    execution: string
): void {
    it(`Should throw if a handler returns rejected promise`, async () => {
        const handlerA = sinon
            .stub()
            .returns(Promise.reject(new Error("my-exception")));
        const handlerB = sinon.stub().returns({ b: "x" });
        const factories = [
            createHandlerFactory("a", () => true, handlerA),
            createHandlerFactory("b", () => false, handlerB),
